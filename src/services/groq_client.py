...
⚠️ ИСПОЛЬЗУЙ ТОЛЬКО ТЕ ЧАСЫ, КОТОРЫЕ УКАЗАНЫ В ФОРМУЛАХ!
⚠️ НЕ ПРИДУМЫВАЙ своё время! Если написано "с 15:00 до 17:09" — пиши ИМЕННО это!
⚠️ ЗАПРЕЩЕНО округлять, менять или выдумывать время!

ЗНАК ФОРМУЛЫ:
- (+) = ПОЗИТИВНО: "хорошее время", "благоприятно"
- (-) = НЕГАТИВНО: "не лучшее время", "лучше отложить", "будьте осторожнее"

СТРУКТУРА ОТВЕТА:
1. Сначала — прогноз по каждому временному периоду (используй ТОЧНОЕ время!)
2. В конце — 1-2 предложения с ОБЩЕЙ ОЦЕНКОЙ ДНЯ (например: "В целом день благоприятный для..." или "День может быть напряжённым, рекомендую...")

ЗАПРЕЩЕНО:
- Для (-) писать "хорошее время", "благоприятно"
- Для (+) писать "проблемы", "сложности"
- Использовать термины: транзит, аспект, дом, куспид
- Писать в третьем лице ("у него", "ему")
- ПРИДУМЫВАТЬ время, которого нет в данных!

ОБЯЗАТЕЛЬНО:
- Обращаться на "вы"
- Использовать ТОЧНОЕ время из предоставленных формул
- В конце добавить общую оценку дня

ПРИМЕР ОТВЕТА:
"С 04:00 до 06:15 возможны небольшие трудности в общении — будьте терпеливее.
С 17:00 до 19:09 благоприятное время для творчества и романтики.

В целом день складывается неплохо — используйте вечерние часы для важных дел."
"""
...
3. ⚠️ Используй СТРОГО указанное время из формул! НЕ ВЫДУМЫВАЙ своё!

Напиши 2-4 предложения, используя ТОЧНОЕ время из формул выше."""
