...
PD_CONSENT_TEXT = """👋 Добро пожаловать!

Для использования бота необходимо Ваше согласие на обработку персональных данных.

Ознакомьтесь с документами:
📄 <a href="https://disk.yandex.ru/i/INC_GXZ5ZzJ1_w">Политика обработки ПД</a>
📄 <a href="https://disk.yandex.ru/i/J8_SAQJ9b5Ewcg">Текст согласия</a>

Нажимая кнопку, Вы подтверждаете ознакомление с документами и даёте согласие на обработку персональных данных."""

MARKETING_CONSENT_TEXT = """📬 <b>Подписка на рассылку</b>

Хотите получать:
• 🔮 Еженедельные астрологические обзоры
• ⭐ Информацию о важных астрологических событиях
• 💎 Специальные предложения и акции

Вы всегда сможете отписаться в настройках."""

PRIVACY_SETTINGS_TEXT = """📄 <b>Документы</b>

Здесь вы можете управлять своими данными и согласиями.

📬 <b>Рассылка:</b> {marketing_status}

⚠️ Удаление данных приведёт к полной очистке вашего профиля и потребует повторного согласия для использования бота."""

DELETE_DATA_CONFIRM_TEXT = """⚠️ <b>Удаление данных</b>

Вы уверены, что хотите удалить все свои данные?

<b>Будут удалены:</b>
• Ваши натальные данные
• История прогнозов
• Диалоги с AI
• Обращения в поддержку
• Подписка

<b>НЕ будут удалены:</b>
• Запись о согласии (требование закона)

После удаления вам потребуется заново дать согласие на обработку ПД."""

DATA_DELETED_TEXT = """✅ <b>Данные удалены</b>

Все ваши персональные данные были удалены из нашей системы.

Для повторного использования бота отправьте /start"""
...
async def start_handler(client: Client, message: Message):
    ...
    # Проверяем, был ли пользователь заблокирован (разблокировал бота)
    if user.is_bot_blocked:
        mark_bot_unblocked(user)

    # Проверка согласия на ПД (152-ФЗ) — ОБЯЗАТЕЛЬНО
    if not has_pd_consent(user):
        await message.reply(
            PD_CONSENT_TEXT,
            reply_markup=get_pd_consent_keyboard(),
...
async def callback_handler(client: Client, callback: CallbackQuery):
    ...
    # === СОГЛАСИЯ (152-ФЗ, 38-ФЗ) ===
    ...
    elif data == "consent_pd_yes":
        # Согласие на обработку ПД
        await callback.answer("Согласие принято!")
        grant_pd_consent(user)
        ...
        # Показать юридические документы (из экрана согласия)
        await callback.answer()
        await callback.message.edit_text(
            "📄 <b>Юридические документы</b>\n\n"
            "Ознакомьтесь с документами по ссылкам ниже:",
            reply_markup=get_documents_keyboard()
        )

    elif data == "back_to_pd_consent":
        # Вернуться к экрану согласия на ПД
        await callback.answer()
        await callback.message.edit_text(
            PD_CONSENT_TEXT,
            reply_markup=get_pd_consent_keyboard(),
    ...
    elif data == "consent_marketing_yes":
        # Согласие на рассылку
        await callback.answer("Вы подписаны на рассылку!")
        grant_marketing_consent(user)
        await show_main_menu(callback, user)

    elif data == "consent_marketing_no":
        # Отказ от рассылки
        await callback.answer()
        refuse_marketing_consent(user)
        await show_main_menu(callback, user)

    elif data == "settings:privacy":
        # Настройки приватности
        await callback.answer()
        marketing_status = "Включена ✅" if user.marketing_consent is True else \
                          "Отключена ❌" if user.marketing_consent is False else "Не настроено"
        await callback.message.edit_text(
            PRIVACY_SETTINGS_TEXT.format(marketing_status=marketing_status),
            reply_markup=get_privacy_settings_keyboard(user.marketing_consent)
        )

    elif data == "show_documents_settings":
        # Показать юридические документы (из настроек приватности)
        await callback.answer()
        from utils.keyboards import get_documents_keyboard as get_docs_kb
        # Создаём клавиатуру с возвратом в настройки приватности
        buttons = [
            [InlineKeyboardButton("📋 Договор-оферта", url="https://disk.yandex.ru/d/CTe2fBbWrwolfg")],
            [InlineKeyboardButton("🔒 Политика обработки ПД", url="https://disk.yandex.ru/i/INC_GXZ5ZzJ1_w")],
            [InlineKeyboardButton("✉️ Согласие на обработку ПД", url="https://disk.yandex.ru/i/J8_SAQJ9b5Ewcg")],
            [InlineKeyboardButton("📬 Согласие на рассылку", url="https://disk.yandex.ru/i/TvOndS5KmoUsyw")],
            [InlineKeyboardButton("↩️ Назад", callback_data="settings:privacy")]
        ]
        await callback.message.edit_text(
            "📄 <b>Юридические документы</b>\n\n"
            "Ознакомьтесь с документами по ссылкам ниже:",
        ...
        # Показать текущий статус рассылки (для тех кто ещё не решил)
        await callback.answer()
        await callback.message.edit_text(
    ...
    elif data == "marketing_on":
        # Включить рассылку
        await callback.answer("Рассылка включена!")
        grant_marketing_consent(user)
        marketing_status = "Включена ✅"
        await callback.message.edit_text(
            PRIVACY_SETTINGS_TEXT.format(marketing_status=marketing_status),
            reply_markup=get_privacy_settings_keyboard(True)
        )

    elif data == "marketing_off":
        # Отключить рассылку
        await callback.answer("Рассылка отключена")
        revoke_marketing_consent(user)
        marketing_status = "Отключена ❌"
        await callback.message.edit_text(
            PRIVACY_SETTINGS_TEXT.format(marketing_status=marketing_status),
            reply_markup=get_privacy_settings_keyboard(False)
        )
        ...
        await callback.answer()
        await callback.message.edit_text(
            DELETE_DATA_CONFIRM_TEXT,
            reply_markup=get_delete_data_keyboard()
        )

    elif data == "delete_data_yes":
        # Удаление данных
        await callback.answer("Данные удаляются...")
    ...
    elif data == "restart_after_delete":
        # Перезапуск после удаления данных — показываем согласие
        await callback.answer()
        await callback.message.edit_text(
            PD_CONSENT_TEXT,
            reply_markup=get_pd_consent_keyboard(),
...
def register_handlers(app: Client):
    ...
    callback_filter = filters.regex(
        ...
        r"consent_pd_yes|consent_marketing_yes|consent_marketing_no|"
        r"show_documents|back_to_pd_consent|settings:privacy|show_documents_settings|"
        r"toggle_marketing|marketing_on|marketing_off|"
        r"delete_data_confirm|delete_data_yes|restart_after_delete)"
    )
