...
from config import (
    ADMIN_USERNAME, SUBSCRIPTION_PRICE, ADMIN_ID, WEBAPP_URL,
    DOCS_PD_CONSENT, DOCS_PRIVACY_POLICY, DOCS_MARKETING_CONSENT, DOCS_OFFER
)
...
def get_admin_main_keyboard(support_count: int = 0) -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        [InlineKeyboardButton("📬 Запрос согласия на рассылку", callback_data="adm_marketing_request")],
...
def get_admin_marketing_audience_keyboard(
    all_count: int, new_count: int, asked_count: int
) -> InlineKeyboardMarkup:
    """Выбор аудитории для массового запроса согласия на рассылку"""
    buttons = [
        [InlineKeyboardButton(
            f"👥 Все без согласия ({all_count})",
            callback_data="adm_marketing_audience_all"
        )],
        [InlineKeyboardButton(
            f"🆕 Новые ({new_count})",
            callback_data="adm_marketing_audience_new"
        )],
        [InlineKeyboardButton(
            f"🔄 Уже спрашивали ({asked_count})",
            callback_data="adm_marketing_audience_asked"
        )],
        [InlineKeyboardButton("◀️ Назад", callback_data="adm_main")]
    ]
    return InlineKeyboardMarkup(buttons)
...
def get_admin_marketing_text_keyboard() -> InlineKeyboardMarkup:
    """Выбор текста для массового запроса согласия"""
    buttons = [
        [InlineKeyboardButton("📝 Стандартный текст", callback_data="adm_marketing_send")],
...
def get_documents_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура с ссылками на юридические документы"""
    buttons = [
        ...
        [InlineKeyboardButton("↩️ Назад", callback_data="back_to_pd_consent")]
    ]
    return InlineKeyboardMarkup(buttons)


def get_privacy_settings_keyboard(marketing_enabled: bool = None) -> InlineKeyboardMarkup:
    """Клавиатура настроек приватности"""
    buttons = []

    # Статус рассылки
    if marketing_enabled is True:
        buttons.append([InlineKeyboardButton(
            "📬 Рассылка: ВКЛ ✅",
            callback_data="marketing_off"
        )])
    elif marketing_enabled is False:
        buttons.append([InlineKeyboardButton(
            "📬 Рассылка: ВЫКЛ ❌",
            callback_data="marketing_on"
        )])
    else:  # None — ещё не решил
        buttons.append([InlineKeyboardButton(
            "📬 Рассылка: Не настроено",
            callback_data="toggle_marketing"
        )])

    buttons.extend([
        [InlineKeyboardButton("📄 Юридические документы", callback_data="show_documents_settings")],
        [InlineKeyboardButton("🗑 Удалить мои данные", callback_data="delete_data_confirm")],
        [InlineKeyboardButton("↩️ Назад", callback_data="back_main")]
    ])
    return InlineKeyboardMarkup(buttons)


def get_delete_data_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура подтверждения удаления данных"""
    buttons = [
        [InlineKeyboardButton("⚠️ Да, удалить все данные", callback_data="delete_data_yes")],
        [InlineKeyboardButton("❌ Отмена", callback_data="settings:privacy")]
    ]
    return InlineKeyboardMarkup(buttons)
