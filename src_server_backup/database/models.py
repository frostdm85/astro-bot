...
class User(BaseModel):
    ...
    # Согласие на ПД (152-ФЗ) — ОБЯЗАТЕЛЬНО
    pd_consent = BooleanField(default=False)
    pd_consent_at = DateTimeField(null=True)

    # Согласие на рассылку (38-ФЗ) — NULL = не спрашивали, True = да, False = отказ
    marketing_consent = BooleanField(null=True)
    marketing_consent_at = DateTimeField(null=True)
    marketing_asked_count = IntegerField(default=0)  # 0-3 попытки
    marketing_next_ask_at = DateTimeField(null=True)

    # Блокировка бота (отзыв согласия или удаление данных)
    is_bot_blocked = BooleanField(default=False)
    bot_blocked_at = DateTimeField(null=True)
...
class ConsentLog(BaseModel):
    """
    Журнал действий с согласиями (152-ФЗ, 38-ФЗ)
    ...
    user = ForeignKeyField(User, backref='consent_logs', on_delete='RESTRICT')
    action = CharField()  # pd_granted, pd_revoked, marketing_granted, etc.
    details = TextField(null=True)  # JSON с дополнительными данными
...
def init_db():
    ...
    # Миграция: добавляем новые поля согласий для существующих пользователей
    # Выполняется однократно, safe=True в create_tables не добавляет новые колонки
    try:
        from playhouse.migrate import SqliteMigrator, migrate as run_migrate
        migrator = SqliteMigrator(db)

        # Проверяем, есть ли уже поле pd_consent
        cursor = db.execute_sql("PRAGMA table_info(users)")
        columns = [row[1] for row in cursor.fetchall()]

        if 'pd_consent' not in columns:
            logger.info("Миграция: добавляем поля согласий в таблицу users...")
            run_migrate(
                migrator.add_column('users', 'pd_consent', BooleanField(default=False)),
                migrator.add_column('users', 'pd_consent_at', DateTimeField(null=True)),
                migrator.add_column('users', 'marketing_consent', BooleanField(null=True)),
                migrator.add_column('users', 'marketing_consent_at', DateTimeField(null=True)),
                migrator.add_column('users', 'marketing_asked_count', IntegerField(default=0)),
                migrator.add_column('users', 'marketing_next_ask_at', DateTimeField(null=True)),
                migrator.add_column('users', 'is_bot_blocked', BooleanField(default=False)),
                migrator.add_column('users', 'bot_blocked_at', DateTimeField(null=True)),
            )
            # Существующие пользователи получают pd_consent = True (конклюдентное согласие)
            db.execute_sql("UPDATE users SET pd_consent = 1 WHERE pd_consent = 0 OR pd_consent IS NULL")
            logger.info("Миграция полей согласий завершена. Существующим пользователям установлено pd_consent = True")
    except Exception as e:
        # Если миграция уже была выполнена или поля уже есть - игнорируем
        if "duplicate column" not in str(e).lower():
            logger.debug(f"Миграция согласий: {e}")
